    ...
    <div>
        ...
        <Modal v-model="formModal" title="更新/查看" footer-hide :mask-closable="false">
            <Form ref="formData" :model="formData" :rules="formRule" :label-width="120"  :disabled="histModal">
                ...
                <FormItem :label="descMap.tag_type_property" prop="tag_type_property" v-show="PropertyShow">
                    <Select v-model="formData.tag_type_property" filterable>
                        <Option v-for="(item, idx) in TagTypePropertyList" :value="item.id" :key="item.id">{{ item.name }}</Option>
                    </Select>
                </FormItem>
                ...
                <FormItem :label="descMap.ref_link" prop="ref_link">
                    <Input type="text" v-model="formData.ref_link"></Input>
                </FormItem>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            updateRecord(name) {
                this.$refs[name].validate((valid) => {
                    if (!valid) {
                        ...
                        if(this.PropertyShow && !this.formData.tag_type_property){
                        ...
                        if(!this.PropertyShow){
                            postData['tag_type_property'] = null
                        }
                        if (form.id) {
            ...
            // 根据tag_type调整property展示
            triggerProperty(tag_type){
                if(tag_type in this.TypePropertyMap){
                    this.PropertyShow = true
                    this.TagTypePropertyList = this.TypePropertyMap[tag_type]
                }else{
                    this.PropertyShow = false
                    this.TagTypePropertyList = []
                }
                if(!this.PropertyIsValid()){
                    this.formData.tag_type_property = null
                }
            },
            // 分页 - 更新页面
            changePage(pageNum) {
        ...
        watch: {
            'formData.tag_type'(val){
                this.triggerProperty(val)
            },
            deep:true
        },
    }
