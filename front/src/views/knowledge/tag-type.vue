    ...
    <div>
        ...
        <div slot="header" class="f-local-header">
            <Form class="f-local-form" ref="formFilter" :model="formFilter" inline>
                ...
                <FormItem>
                    <Select v-model="formFilter.opt_show" filterable :placeholder="descMap.opt_show">
                        <Option v-for="(item, idx) in BooleanList" :value="item.name" :key="item.name">{{ item.desc }}</Option>
                    </Select>
                </FormItem>
                <FormItem>
                    ...
                    <Button type="info" icon="md-settings" @click="routeProperty">属性管理</Button>
                </FormItem>
            </Form>
        </div>
        ...
        <Modal v-model="formModal" title="更新/查看" footer-hide :mask-closable="false">
            <Form ref="formData" :model="formData" :rules="formRule" :label-width="120" :disabled="histModal">
                ...
                <FormItem :label="descMap.desc" prop="desc">
                    <Input type="textarea" cols="40" v-model="formData.desc"></Input>
                </FormItem>
                <FormItem :label="descMap.select_type" prop="select_type">
                    <Select v-model="formData.select_type" filterable>
                        <Option v-for="(item, idx) in selectTypeList" :value="item.name" :key="item.name">{{ item.desc }}</Option>
                    </Select>
                </FormItem>
                <FormItem :label="descMap.required" prop="required">
                    <Select v-model="formData.required" filterable>
                        <Option v-for="(item, idx) in BooleanList" :value="item.name" :key="item.name">{{ item.desc }}</Option>
                    </Select>
                </FormItem>
                <FormItem :label="descMap.opt_show" prop="opt_show">
                    <Select v-model="formData.opt_show" filterable>
                        <Option v-for="(item, idx) in BooleanList" :value="item.name" :key="item.name">{{ item.desc }}</Option>
                    </Select>
                </FormItem>
                <FormItem :label="descMap.statistic_show" prop="statistic_show">
                    <Select v-model="formData.statistic_show" filterable>
                        <Option v-for="(item, idx) in BooleanList" :value="item.name" :key="item.name">{{ item.desc }}</Option>
                    </Select>
                </FormItem>
                <FormItem :label="descMap.column_show" prop="column_show">
                    <Select v-model="formData.column_show" filterable>
                        <Option v-for="(item, idx) in BooleanList" :value="item.name" :key="item.name">{{ item.desc }}</Option>
                    </Select>
                </FormItem>
        ...
        methods: {
            ...
            loadTableData(page) {
                ...
                requestAPI(this.url, params).then(resp => {
                    ...
                    this.tableColumns = []
                    this.BooleanList = resp.data.boolean_list
                    this.selectTypeList = resp.data.select_type_list
