    ...
    <div>
        ...
        <div slot="header" class="f-local-header">
            <Form class="f-local-form" ref="formFilter" :model="formFilter" :rules="formFilterRule" inline>
                <FormItem prop="organ">
                    <Input type="text" v-model="formFilter.title" :placeholder="descMap.title"></Input>
                </FormItem>
                <FormItem prop="organ">
        ...
        >
            <template slot-scope="{ row }" slot="title">
                <a v-if="row.title_url && row.title_url.trim().length > 0" :href="row.title_url.trim()" target="_blank">{{row.title}}</a>
                <span v-else>{{row.title}}</span>
            </template>
            <template slot-scope="{ row }" slot="pub_time">
                <span v-if="row.pub_time && row.pub_time.length > 0">{{row.pub_time && row.pub_time.substr(0, 10)}}</span>
               <span v-else>-</span>
            </template>
            <template slot-scope="{ row }" slot="interpretation">
                <a v-if="row.inter_url && row.inter_url.trim().length > 0" :href="row.inter_url.trim()" target="_blank">{{row.interpretation}}</a>
                <span v-else>{{row.interpretation}}</span>
            </template>
...
<!--                <Button type="primary" size="small" @click="showLink(index)">链接</Button>-->
        ...
        <Modal v-model="formModal" title="变更" footer-hide>
            <Form ref="formData" :model="formData" :rules="formRule" :label-width="120">
                ...
                <FormItem label="政策法规" prop="title">
                    <Input type="text" v-model="formData.title"></Input>
                </FormItem>
                <FormItem label="法规链接" prop="source_link">
                    <Input type="text" v-model="formData.title_url"></Input>
                </FormItem>
                ...
                <FormItem label="政策解读" prop="interpretation">
                    <Input type='textarea' :rows="3" v-model="formData.interpretation"></Input>
                </FormItem>
                <FormItem label="解读链接" prop="inter_url">
                    <Input type="text" v-model="formData.inter_url"></Input>
                </FormItem>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                url: '/api/knowledge/policy-trace/',
