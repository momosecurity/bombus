...
<!--                <Button type="error" size="small" @click="removeRecord(index)">删除</Button>-->
        ...
        <Modal v-model="formModal" title="更新/查看" footer-hide width="700px" :mask-closable="false">
            <Form ref="formData" :model="formData" :rules="formRule" :label-width="120">
                ...
                <FormItem label="审阅周期" prop="audit_period">
                    <Select v-model="formData.audit_period" filterable>
                        <Option v-for="(item, idx) in auditPeriodList" :value="item.name" :key="item.name">{{ item.desc }}</Option>
                    </Select>
                </FormItem>
                <FormItem label="策略原子" prop="atoms">
                    <Select v-model="formData.atoms" filterable multiple>
                        <Option v-for="(item, idx) in ruleAtomList" :value="item.id" :key="item.id">{{ item.name }}</Option>
                    </Select>
                </FormItem>
...
<script>
    ...
    export default {
        ...
        methods: {
            // 弹出数据变更表单
            showForm(index) {
                this.getRuleAtomList();
                this.$refs['formData'].resetFields()
                if (index !== undefined) {
                    const url = `/api/audit/rule_group/${this.tableData[index].id}/`
            ...
            getRuleAtomList() {
                const url = `/api/audit/rule_atom/`
                requestAPI(url, {}, 'get').then(resp => {
                    this.ruleAtomList = resp.data.results;
                })
            },
            // 数据过滤
            filterRecord(name) {
            ...
            updateRecord(name) {
                this.$refs[name].validate((valid) => {
                    if (!valid) {
                        ...
                        let postData = {...form}
                        if (form.id) {
                            const url = `/api/audit/rule_group/${form.id}/`
                            requestAPI(url, {}, 'patch', postData).then(resp => {
                                this.$Message.success('更新成功')
                                this.loadTableData()
                            })
                        } else {
                            requestAPI('/api/audit/rule_group/', {}, 'post', postData).then(resp => {
            ...
            loadTableData(page) {
                ...
                requestAPI('/api/audit/rule_group/', params).then(resp => {
                    ...
                    this.auditPeriodList = resp.data.audit_period
