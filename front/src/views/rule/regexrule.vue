    ...
    <div>
        ...
        <div slot="header" class="f-local-header">
            <Form class="f-local-form" ref="formFilter" :model="formFilter" :rules="formFilterRule" inline>
                ...
                <FormItem prop="regex">
                    <Input type="text" v-model="formFilter.regex" :placeholder="descMap.regex"></Input>
                </FormItem>
...
<!--                <Button type="error" size="small" @click="removeRecord(index)">删除</Button>-->
        ...
        <Modal v-model="formModal" title="更新/查看" footer-hide width="700px" :mask-closable="false">
            <Form ref="formData" :model="formData" :rules="formRule" :label-width="120">
                ...
                <FormItem label="正则表达式" prop="regex">
                    <Input type="text" v-model="formData.regex"></Input>
                </FormItem>
    ...
    export default {
        ...
        methods: {
            ...
            showForm(index) {
                ...
                if (index !== undefined) {
                    const url = `/api/audit/regex_rule/${this.tableData[index].id}/`
            ...
            updateRecord(name) {
                this.$refs[name].validate((valid) => {
                    if (!valid) {
                        ...
                        if (form.id) {
                            const url = `/api/audit/regex_rule/${form.id}/`
                            requestAPI(url, {}, 'patch', postData).then(resp => {
                                this.$Message.success('更新成功')
                                this.loadTableData()
                            })
                        } else {
                            requestAPI('/api/audit/regex_rule/', {}, 'post', postData).then(resp => {
            ...
            loadTableData(page) {
                ...
                requestAPI('/api/audit/regex_rule/', params).then(resp => {
